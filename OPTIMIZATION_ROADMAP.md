...
- 性能测试（O1）：60/60 pass，median_sum ≈ 96,000 ms（偏高）
- 已实现：Mem2Reg、mm_hoist、loop_interchange、loop_tiling、RowScratchMatmul（AST 版）、双层 IR 骨架（HIR）
- 主要瓶颈：shuffle1（~9200 ms）、h-5* 系列（~4300 ms）、h-8*、conv2d-1、many_mat_cal*
...
## 当前优先任务（2026.05.22 之后）

1. **RowScratchMatmul 真正落地**（HIR 版 + AST 版联动）
2. **RV StrengthReduct** 实现（先魔法除法，再 mul 分解）
3. **实现真正的 `lowerToLegacyIR`**（让双层 IR 跑通）
4. **后端简单 peephole**（load-load CSE）
5. 跑通一次完整性能评测并记录关键用例数据
