...
- **评测编译命令**：官方使用 `clang++` 递归扫描 `.h/.hpp/.c/.cc/.cpp/...` 并链接为 `compiler`。请避免在同仓库混入其它带 `main` 的可编译文件。
...
也可以直接使用评测环境的 C++17 编译器（与 `Makefile` 等价的多文件编译）：
...
clang++ -std=c++17 -O2 -o compiler main.o common.o lexer.o parser.o semantic.o codegen.o
```
...
| `parser.*` | 递归下降语法分析 |
| `semantic.*` | 语义分析与常量折叠 |
| `codegen.*` | RISC-V64 汇编生成 |
| `ir.h` | IR 中端占位（后续接 SSA / 优化流水线） |
...
命令行会正确识别 **`-O1`**；语义阶段已包含基础常量折叠，后续可在保留该开关的前提下实现 **`ir.h` 所载 IR** 与更多 `-O1` 优化，并把选项传入中端或后端。
