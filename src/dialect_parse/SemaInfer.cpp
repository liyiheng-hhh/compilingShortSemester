// compiler2026-x phase-A (dialect_parse sema)
...
Type *Sema::infer(ASTNode *node) {
  ...
  if (auto binary = dyn_cast<BinaryNode>(node)) {
    ...
    if (isa<VoidType>(lty) || isa<VoidType>(rty))
      semFail("void value used in binary expression");
    ...
    if (lty != rty) {
      semFail("incompatible operand types in binary operation");
    }
  ...
  if (auto unary = dyn_cast<UnaryNode>(node)) {
    ...
    if (isa<VoidType>(ty))
      semFail("void value used in unary expression");
  ...
  if (auto vardecl = dyn_cast<VarDeclNode>(node)) {
    ...
    if (vardecl->global || !vardecl->mut)
      ...
      if (ty != vardecl->type) {
        semFail("incompatible initializer type");
      }
    }
    return ctx.create<VoidType>();
  }
  ...
  if (auto ret = dyn_cast<ReturnNode>(node)) {
    ...
    if (!ret->node) {
      if (!isa<VoidType>(retTy))
        semFail("non-void function must return a value");
      return ctx.create<VoidType>();
    }
    if (isa<VoidType>(retTy))
      semFail("void function cannot return a value");
  ...
  if (auto ref = dyn_cast<VarRefNode>(node)) {
    if (!symbols.count(ref->name)) {
      semFail("cannot find symbol " + ref->name);
    }
  ...
  if (auto loop = dyn_cast<WhileNode>(node)) {
    ...
    if (isa<FloatType>(condTy)) {
      ...
      semFail("while condition must be int/float");
    }
    infer(loop->body);
    return node->type = ctx.create<VoidType>();
  }
  ...
  if (auto assign = dyn_cast<AssignNode>(node)) {
    ...
    if (!lref)
      semFail("left side of assignment must be variable");
    if (!isMutableSymbol(lref->name))
      semFail("cannot assign to const variable " + lref->name);

    auto lty = infer(assign->l);
    auto rty = infer(assign->r);
    if (isa<VoidType>(rty))
      semFail("void value cannot be assigned");
    if (isa<IntType>(lty) && isa<FloatType>(rty)) {
    ...
    if (lty != rty) {
      semFail("assignment type mismatch");
    }
    return node->type = ctx.create<VoidType>();
  }
  ...
  if (auto call = dyn_cast<CallNode>(node)) {
    if (!symbols.count(call->func))
      semFail("cannot find function " + call->func);
    auto *symTy = symbols[call->func];
    if (!isa<FunctionType>(symTy))
      semFail(call->func + " is not a function");
    ...
    for (size_t i = 0; i < fnTy->params.size(); i++) {
      ...
      if (isa<VoidType>(ty))
        semFail("void value cannot be used as function argument");
  ...
  if (auto access = dyn_cast<ArrayAccessNode>(node)) {
    if (!symbols.count(access->array))
      semFail("cannot find array symbol " + access->array);
    ...
    if (isa<ArrayType>(realTy))
      ...
      semFail(access->array + " is not array/pointer");
    ...
    for (auto x : access->indices) {
      auto ty = infer(x);
      if (!isa<IntType>(ty))
        semFail("array index must be int");
    }
  ...
  if (auto write = dyn_cast<ArrayAssignNode>(node)) {
    if (!symbols.count(write->array))
      semFail("cannot find array symbol " + write->array);
    ...
    if (isa<ArrayType>(realTy))
      ...
      semFail(write->array + " is not array/pointer");

    auto baseTy = arrTy->base;
    if (write->indices.size() != arrTy->dims.size())
      semFail("array assignment index rank mismatch");
    write->arrTy = arrTy;
    ...
    if (isa<VoidType>(valueTy))
      semFail("void value cannot be assigned to array element");
  ...
  semFail("cannot infer AST node kind");
}
