#include "PrePasses.h"
#include "../opt/Passes.h"

#include <map>

using namespace sys;

#define INT(op) isa<IntOp>(op)

static bool loadEscapesToCall(Op *load) {
  for (auto use : load->getUses()) {
    if (isa<CallOp>(use))
      return true;
  }
  return false;
}

static bool hasStoresTo(Op *op) {
  for (auto use : op->getUses()) {
    if (isa<StoreOp>(use))
      return true;

    if (isa<AddIOp>(use) || isa<AddLOp>(use)) {
      if (hasStoresTo(use))
        return true;
      continue;
    }

    if (isa<LoadOp>(use))
      continue;

    return false;
  }

  return false;
}

std::map<std::string, int> EarlyConstFold::stats() {
  return {
    { "folded-ops", foldedTotal }
  };
}

int EarlyConstFold::foldImpl() {
  Builder builder;

  // Do not run RegularFold here: it may erase duplicate IntOps that are still
  // distinct SSA values (e.g. after global/alloca const-prop), which triggers
  // Op::erase() assertions. The dedicated RegularFold pipeline pass handles cleanup.
  int folded = 0;

  bool changed;
  std::vector<Op*> ifs;
  do {
    changed = false;
    ifs = module->findAll<IfOp>();
    for (auto op : ifs) {
      auto cond = op->getOperand().defining;

      if (!INT(cond))
        continue;

      folded++;
      changed = true;
      auto region = V(cond) ? 0 : 1;
      if (op->getRegions().size() > region) {
        auto ifso = op->getRegion(region);
        for (auto bb : ifso->getBlocks()) {
          auto ops = bb->getOps();
          for (auto inner : ops)
            inner->moveBefore(op);
        }
      }

      op->erase();
      break;
    }
  } while (changed);

  return folded;
}

void EarlyConstFold::run() {
  int folded = 0;

  Builder builder;
  
  auto getglobs = module->findAll<GetGlobalOp>();
  auto gMap = getGlobalMap();
  auto fMap = getFunctionMap();
  std::set<GlobalOp*> nonConst;

  for (auto op : getglobs) {
    const auto &name = NAME(op);
    if (hasStoresTo(op)) {
      nonConst.insert(gMap[name]);
      continue;
    }

    for (auto use : op->getUses()) {
      if (isa<CallOp>(use) && (beforePureness || isExtern(NAME(use)) || fMap[NAME(use)]->has<ImpureAttr>())) {
        nonConst.insert(gMap[name]);
        break;
      }
      if (isa<LoadOp>(use) && loadEscapesToCall(use)) {
        nonConst.insert(gMap[name]);
        break;
      }
    }
  }

  do {
    auto allocas = module->findAll<AllocaOp>();
  
    for (auto alloca : allocas) {
      auto uses = alloca->getUses();
      Op *store = nullptr;
      bool good = true;
      for (auto use : uses) {
        if (isa<LoadOp>(use)) {
          if (loadEscapesToCall(use)) {
            good = false;
            break;
          }
          continue;
        }
        
        if (isa<StoreOp>(use) && use->DEF(1) == alloca && !store) {
          store = use;
          continue;
        }
  
        good = false;
        break;
      }
  
      if (!good || !store)
        continue;
  
      Op *def = store->DEF(0);
  
      if (isa<LoadOp>(def) || isa<CallOp>(def) || isa<GetArgOp>(def))
        continue;
      
      for (auto use : uses) {
        if (isa<LoadOp>(use)) {
          use->replaceAllUsesWith(def);
          use->erase();
        }
      }
  
      store->erase();
      alloca->erase();
    }

    folded = foldImpl();

    getglobs = module->findAll<GetGlobalOp>();
    for (auto get : getglobs) {
      const auto &name = NAME(get);
      auto global = gMap[name];
      if (nonConst.count(global))
        continue;

      auto uses = get->getUses();
      std::vector<Op*> scalarLoads;
      for (auto use : uses) {
        assert(!isa<StoreOp>(use));
        if (isa<LoadOp>(use))
          scalarLoads.push_back(use);
      }

      if (!scalarLoads.empty()) {
        folded++;
        builder.setBeforeOp(scalarLoads[0]);
        Op *value;
        if (auto intArr = global->find<IntArrayAttr>())
          value = builder.create<IntOp>({ new IntAttr(intArr->vi[0]) });
        else
          value = builder.create<FloatOp>({ new FloatAttr(global->get<FloatArrayAttr>()->vf[0]) });

        for (auto load : scalarLoads) {
          load->replaceAllUsesWith(value);
          load->erase();
        }
      }

      for (auto use : uses) {
        if (isa<LoadOp>(use))
          continue;

        if (isa<AddLOp>(use)) {
          auto y = use->getOperand(1).defining;
          if (!INT(y))
            continue;

          auto targets = use->getUses();
          std::map<int, std::vector<Op*>> loadsByOffset;
          for (auto target : targets) {
            assert(!isa<StoreOp>(target));

            auto size = SIZE(global);
            if (V(y) >= size) {
              std::cerr << "warning: out of bounds access\n";
              std::cerr << "array has " << size / 4 << " elements, but accessing subscript " << V(y) / 4 << "\n";
              continue;
            }

            if (isa<LoadOp>(target))
              loadsByOffset[V(y) / 4].push_back(target);
          }

          for (auto [offset, loads] : loadsByOffset) {
            folded++;
            builder.setBeforeOp(use);
            Op *value;
            if (auto intArr = global->find<IntArrayAttr>())
              value = builder.create<IntOp>({ new IntAttr(intArr->vi[offset]) });
            else
              value = builder.create<FloatOp>({ new FloatAttr(global->get<FloatArrayAttr>()->vf[offset]) });
            for (auto load : loads) {
              load->replaceAllUsesWith(value);
              load->erase();
            }
          }
        }
      }
    }

    for (auto op : getglobs) {
      if (!op->getUses().size())
        op->erase();
    }

    foldedTotal += folded;
  } while (folded);
}
