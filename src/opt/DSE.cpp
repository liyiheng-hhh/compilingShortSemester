...
void DSE::removeUnread(Op *op, const std::vector<Op*> &gets) {
  // Only const globals may be store-only; mutable globals (e.g. crypto `state`)
  // can become store-only after inlining while still being semantically live.
  if (op->has<ImpureAttr>() || !op->has<ConstAttr>())
    return;
